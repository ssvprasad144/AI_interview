...
from .services import get_ai_response, generate_feedback, generate_hint
...
def voice_next_question(request):
    ...
    try:
        ...
        return JsonResponse({"error": "Interview session not found. Please restart the interview."}, status=404)

    intro_text = "Hi, welcome to NIT Jamshedpur interview. Please introduce yourself briefly."
    ...
    # Persist a lightweight local summary to avoid extra LLM latency in request path.
    try:
    ...
    try:
        ...
        # Fallback keeps interview flow alive even if AI provider is slow/unavailable.
        fallback_questions = [
            "Can you explain a recent project you worked on and the challenges you solved?",
            "How would you design a scalable API endpoint for high traffic?",
            "What trade-offs do you consider when choosing data structures in production code?",
            "Describe a bug you faced recently and how you debugged it step by step.",
        ]
        ai_question = fallback_questions[ai_messages % len(fallback_questions)]
