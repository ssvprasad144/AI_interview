...
import textwrap
from .services import generate_feedback, generate_reference_answer
import tempfile
import json
...
def ai_interview_live(request):
    ...
    try:
        InterviewSession.objects.get(id=session_id)
...
def stt_transcribe(request):
    ...
    try:
        ...
        logger.exception("stt_transcribe failed: %s", exc)
        return JsonResponse({
            "error": "Speech transcription is temporarily unavailable. Please try again or use text input.",
...
@require_GET
def reference_answer(request, session_id, question_index):
    session = InterviewSession.objects.get(id=session_id)
    ...
    context = {
        "role": request.session.get("interview_role") or session.interview_type,
        "stack": request.session.get("interview_stack", []),
        "difficulty": request.session.get("interview_difficulty", "standard"),
    }
    answer = generate_reference_answer(question, context)
