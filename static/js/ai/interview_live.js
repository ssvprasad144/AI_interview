document.addEventListener("DOMContentLoaded", () => {
    ...
    let interviewRecordingUrl = null;
    let interviewRecordingMimeType = "video/webm";
    let interviewRecordingActive = false;
    ...
    let selectedCameraId = localStorage.getItem("aiInterviewCameraDeviceId") || "";
    ...
    const cameraSourceHint = document.getElementById("cameraSourceHint");
    const recordInterviewToggle = document.getElementById("recordInterviewToggle");
    const recordPromptModal = document.getElementById("recordPromptModal");
    const startWithRecordingBtn = document.getElementById("startWithRecordingBtn");
    const startWithoutRecordingBtn = document.getElementById("startWithoutRecordingBtn");
    const downloadRecordingBtn = document.getElementById("downloadRecordingBtn");
    const recordingStatus = document.getElementById("recordingStatus");

    function getRecordFileName() {
        const stamp = new Date().toISOString().replace(/[:.]/g, "-");
        return `ai-interview-${SESSION_ID || "session"}-${stamp}.webm`;
    }
    ...
    function showRecordPrompt() {
        if (!recordPromptModal) return;
        recordPromptModal.classList.add("show");
        recordPromptModal.setAttribute("aria-hidden", "false");
    }

    function hideRecordPrompt() {
        if (!recordPromptModal) return;
        recordPromptModal.classList.remove("show");
        recordPromptModal.setAttribute("aria-hidden", "true");
    }

    function clearRecordingUrl() {
        if (interviewRecordingUrl) {
            URL.revokeObjectURL(interviewRecordingUrl);
    ...
    function updateDownloadButtonState() {
        ...
        const hasVideo = interviewRecordingChunks.length > 0;
        downloadRecordingBtn.style.display = hasVideo ? "inline-flex" : "none";
        downloadRecordingBtn.disabled = !hasVideo;
    }

    function triggerRecordingDownload() {
        if (!interviewRecordingChunks.length) return false;
        clearRecordingUrl();
        const blob = new Blob(interviewRecordingChunks, { type: interviewRecordingMimeType || "video/webm" });
        interviewRecordingUrl = URL.createObjectURL(blob);

        const link = document.createElement("a");
        link.href = interviewRecordingUrl;
        link.download = getRecordFileName();
    ...
    async function startInterviewRecording() {
        if (!recordInterviewToggle?.checked) return;
        if (interviewRecordingActive || interviewFinished) return;
        if (typeof MediaRecorder === "undefined") {
            setRecordingStatus("Not Supported", false);
            showActionToast("Browser does not support recording");
            return;
        }
        ...
        if (!videoTrack || !audioTrack) {
            setRecordingStatus("Waiting for Media", false);
            return;
        }

        const mimeType = getSupportedRecordingMimeType();
        interviewRecordingMimeType = mimeType || "video/webm";
        interviewRecordingStream = new MediaStream([videoTrack, audioTrack]);

        interviewRecorder = mimeType
            ? new MediaRecorder(interviewRecordingStream, { mimeType })
            : new MediaRecorder(interviewRecordingStream);

        interviewRecorder.ondataavailable = (e) => {
            if (e.data && e.data.size > 0) {
                interviewRecordingChunks.push(e.data);
                updateDownloadButtonState();
            }
        };

        interviewRecorder.onstop = () => {
            interviewRecordingActive = false;
        ...
        interviewRecordingActive = true;
        setRecordingStatus("Recording On", true);
    }
    ...
    async function beginInterview(recordEnabled) {
        if (startInProgress || interviewStarted || interviewFinished) return;
        startInProgress = true;

        if (recordInterviewToggle) {
            recordInterviewToggle.checked = !!recordEnabled;
            localStorage.setItem("aiInterviewRecordEnabled", String(!!recordEnabled));
        }
        ...
        try {
            if (!SESSION_ID) {
                throw new Error("Interview session not found. Please restart from interview setup.");
            }

            if (aiTextEl) {
                aiTextEl.textContent = "Starting interview...";
            }
            ...
            if (!introCompleted) {
                ...
                if (!introRes.ok) {
                    throw new Error(introData.error || "Failed to start interview.");
                }
            ...
            startPauseBtn.textContent = "Start Interview";
            pauseTimer();
            stopCamera();
            stopRecording();
            if (stream) {
            ...
            if (aiTextEl) {
                aiTextEl.textContent = err?.message || "Unable to start interview. Please allow camera/microphone and try again.";
            }
            setAIState("idle");
        } finally {
    ...
    /* ================= CAMERA ================= */
    function updateCameraHint(message) {
        if (!cameraSourceHint) return;
        cameraSourceHint.textContent = message;
    }
    ...
    function populateCameraSelect() {
        ...
        autoOption.value = "";
        autoOption.textContent = "Auto detect camera";
        ...
        if (selectedCameraId && availableCameras.some(d => d.deviceId === selectedCameraId)) {
    ...
    async function grantVideoPermissionForLabels() {
        try {
            const temp = await navigator.mediaDevices.getUserMedia({ video: true, audio: false });
            temp.getTracks().forEach(track => track.stop());
        } catch (e) {
            updateCameraHint("Allow camera permission to list camera names.");
        }
    }

    async function refreshCameraList(requestLabels = false) {
        if (!navigator.mediaDevices || !navigator.mediaDevices.enumerateDevices) {
            if (cameraSelect) cameraSelect.disabled = true;
            if (switchCameraBtn) switchCameraBtn.disabled = true;
            if (refreshCamerasBtn) refreshCamerasBtn.disabled = true;
            updateCameraHint("Camera device detection is not supported in this browser.");
            return;
        }

        if (requestLabels) {
            await grantVideoPermissionForLabels();
        }
        ...
        if (selectedCameraId && !availableCameras.some(d => d.deviceId === selectedCameraId)) {
            selectedCameraId = "";
            localStorage.removeItem("aiInterviewCameraDeviceId");
        }

        if (cameraSelect) cameraSelect.disabled = availableCameras.length === 0;
        if (switchCameraBtn) switchCameraBtn.disabled = availableCameras.length === 0;
        populateCameraSelect();

        if (!availableCameras.length) {
            updateCameraHint("No camera detected. Connect a camera and click Refresh.");
            return;
        }

        if (selectedCameraId) {
            const selected = availableCameras.find(d => d.deviceId === selectedCameraId);
            updateCameraHint(`Selected camera: ${selected ? cameraDisplayName(selected, 0) : "Custom"}`);
            return;
        }

        updateCameraHint("Select a camera and click Switch. If labels are missing, click Refresh after permission.");
    }
    ...
    async function startCamera(forceRestart = false) {
        ...
        try {
            ...
            // If selected device is unavailable, fall back to browser default camera.
            if (selectedCameraId) {
                selectedCameraId = "";
                localStorage.removeItem("aiInterviewCameraDeviceId");
                ...
                updateCameraHint("Selected camera was unavailable, switched to default camera.");
            } else {
                throw err;
            }
        }
        ...
        if (activeDeviceId) {
            ...
            localStorage.setItem("aiInterviewCameraDeviceId", activeDeviceId);
        ...
        if (selectedCameraId) {
            const selected = availableCameras.find(d => d.deviceId === selectedCameraId);
            updateCameraHint(`Live camera: ${selected ? cameraDisplayName(selected, 0) : "Selected device"}`);
        } else {
            updateCameraHint("Live camera started.");
        }
    ...
    if (recordInterviewToggle) {
        const savedRecordPref = localStorage.getItem("aiInterviewRecordEnabled") === "true";
        recordInterviewToggle.checked = savedRecordPref;
        setRecordingStatus(savedRecordPref ? "Ready to Record" : "Recording Off", false);
        recordInterviewToggle.addEventListener("change", async () => {
            localStorage.setItem("aiInterviewRecordEnabled", String(recordInterviewToggle.checked));
            if (!recordInterviewToggle.checked) {
                await stopInterviewRecording();
                setRecordingStatus("Recording Off", false);
                return;
            }
            setRecordingStatus("Ready to Record", false);
            if (interviewStarted) {
                await startInterviewRecording();
            }
        });
    ...
    if (startWithRecordingBtn) {
        startWithRecordingBtn.addEventListener("click", async () => {
            await beginInterview(true);
        });
    }

    if (startWithoutRecordingBtn) {
        startWithoutRecordingBtn.addEventListener("click", async () => {
    ...
    if (downloadRecordingBtn) {
        downloadRecordingBtn.addEventListener("click", () => {
            const ok = triggerRecordingDownload();
            if (!ok) showActionToast("No recording available yet");
        });
    }
    ...
    async function processAnswer(durationMs) {
        ...
        if (!transcript) {
            ...
            try {
                ...
                if (!sttRes.ok) {
                    const msg = await parseErrorMessage(sttRes, "Transcription request failed.");
                    throw new Error(msg);
                }
                ...
                try {
                    ...
                    if (!retryRes.ok) {
                        const msg = await parseErrorMessage(retryRes, "Transcription request failed.");
                        throw new Error(msg);
                    }
                    sttData = await retryRes.json();
                } catch (retryErr) {
                    aiTextEl.textContent = retryErr?.message || "Transcription failed. Please answer again.";
                    setAIState("listening");
                    startRecording();
                    return;
                }
        ...
        try {
            ...
            if (!aiRes.ok) {
                const msg = await parseErrorMessage(aiRes, "Could not generate the next question.");
                throw new Error(msg);
            }
            ...
            try {
                ...
                if (!retryRes.ok) {
                    const msg = await parseErrorMessage(retryRes, "Could not generate the next question.");
                    throw new Error(msg);
                }
                aiData = await retryRes.json();
            } catch (retryErr) {
                const isTimeout = retryErr?.name === "AbortError";
                aiTextEl.textContent = isTimeout
                    ? "Request timed out. Please try again."
                    : (retryErr?.message || "Unable to generate next question. Please try again.");
                setAIState("listening");
                startRecording();
                return;
            }
    ...
    async function finishInterview(reason = "exit") {
        ...
        if (needsConfirmation) {
            const confirmed = confirm(
                "If you leave now, the interview will end and feedback will be generated.\n\nDo you want to continue?"
            );
        ...
        await stopInterviewRecording();
        if (recordInterviewToggle?.checked && interviewRecordingChunks.length > 0) {
        ...
        try {
            ...
            throw new Error("Feedback redirect missing");
        } catch {
            finishingInProgress = false;
    ...
    /* ================= START / PAUSE / RESUME ================= */
    if (startPauseBtn) {
        startPauseBtn.onclick = async (event) => {
            if (event && typeof event.preventDefault === "function") {
                event.preventDefault();
            }
            if (startInProgress) return;

            if (!interviewStarted && !introCompleted) {
                showRecordPrompt();
                return;
            }
            ...
            // ▶ START / RESUME
            if (!interviewStarted) {
                await beginInterview(recordInterviewToggle?.checked);
                return;
            }
    ...
    if (skipBtn) {
        skipBtn.addEventListener("click", async () => {
            ...
            try {
                ...
                if (!res.ok) {
                    const msg = await parseErrorMessage(res, "Could not skip question.");
                    throw new Error(msg);
                }
                const data = await res.json();
                ...
                showActionToast(err?.message || "Skip failed");
            }
        });
    }
    ...
    if (cameraSelect) {
        cameraSelect.addEventListener("change", (event) => {
            ...
            if (selectedCameraId) {
                localStorage.setItem("aiInterviewCameraDeviceId", selectedCameraId);
            } else {
                localStorage.removeItem("aiInterviewCameraDeviceId");
            }
        });
    ...
    if (switchCameraBtn) {
        switchCameraBtn.addEventListener("click", async () => {
            ...
            try {
                ...
                updateCameraHint(err?.message || "Unable to switch camera. Check camera permission.");
            } finally {
    ...
    renderRubric();
    refreshCameraList(false).catch(() => {
        updateCameraHint("Unable to detect camera devices right now.");
    });
